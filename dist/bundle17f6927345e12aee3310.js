...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function checkAnswer(wordName, correctDefinition) {
    ...
    var submitButton = document.querySelector('.btn-primary'); // Reference to the submit button

    if (selected) {
      var isCorrect = false;
      var selectedLabel = selected.nextElementSibling;
      if (selectedLabel.textContent === correctDefinition) {
        isCorrect = true;
        score++; // Increment score for a correct answer
        wordBank = wordBank.filter(function (word) {
          return word.name !== wordName;
        }); // Remove the word from the word bank if answered correctly
      } else {
        var delay = Math.floor(Math.random() * 6) + 5; // Random delay between 5 to 10 questions for pending questions
        var currentQuestionIndex = totalQuestions + delay;
        pendingQuestions.splice(currentQuestionIndex, 0, wordBank.find(function (word) {
          return word.name === wordName;
        })); // Add incorrect word to pending questions
      }
...
var multiplier = 1;
var floorCount = 0;
var multiplierBase = 1; // Base multiplier value
...
function updateTower() {
  ...
  var multiplierDisplay = document.getElementById('multiplier-display');
  if (multiplierDisplay) {
    multiplierDisplay.textContent = "Multiplier: x".concat(multiplier, " | Floors: ").concat(floorCount);
  }
  tower.forEach(function (floor) {
...
function answer(correct) {
  if (correct) {
    ...
    if (correctStreak >= 20) multiplier = 5;else if (correctStreak >= 15) multiplier = 4;else if (correctStreak >= 10) multiplier = 3;else if (correctStreak >= 5) multiplier = 2;else multiplier = 1;
    addFloor(multiplier);
  } else {
    correctStreak = 0;
    incorrectStreak++;
...
/******/ })()
;
//# sourceMappingURL=bundle17f6927345e12aee3310.js.map